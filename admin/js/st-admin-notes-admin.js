(function ($) {
	...
	$(function () {
		const data = window.st_admin_notes_data || {};
		...
		const getCanvas = () => $('#st-admin-notes-canvas');
		const $adminTable = $('.st-notes-admin-table');
		const $adminTableBody = $adminTable.find('tbody');
		const $mainForm = $('form').filter((_, el) => ['add_note', 'edit_note'].includes($(el).find('input[name="stn_action"]').val()));
		const $modal = $('#stn-modal');
		const $modalForm = $('#stn-modal-form');
		...
		const card = $('<span class="st-note-card"></span>')
		...
		/* ---------- Header ---------- */
		const $header = $('<div class="st-note-card__header"></div>');
		const $title  = $('<span class="st-note-card__title"></span>').text(note.title || 'Note');

		/* ---------- Buttons ---------- */
		const $minBtn  = $('<button type="button" class="st-note-card__btn st-min" aria-label="Minimize">–</button>');
		const $fullBtn = $('<button type="button" class="st-note-card__btn st-full" aria-label="Fullscreen">⛶</button>');
		const $hideBtn = $('<button type="button" class="st-note-card__btn st-hide" aria-label="Hide">×</button>');

		const $actions = $('<div class="st-note-card__actions"></div>')
		...
		/* ---------- Body ---------- */
		const body = $('<div class="st-note-card__body"></div>')
		.text(note.content || '');
		...
		function updateMinimizedStack() {
			$('.st-note-card.minimized').each(function (index) {
		...
		/* ---------- Drag & Resize ---------- */
		card.draggable({
			handle: '.st-note-card__header',
			containment: '#st-admin-notes-canvas',
			scroll: false,
			disabled: false,
			start: () => {
		...
		const fetchNotes = (done) => {
			ajax({ action: 'stn_fetch_notes' }, (resp) => {
				if (resp && resp.notes) {
		...
		const addNote = (payload, done) => {
			ajax({ action: 'stn_add_note', ...payload }, (resp) => {
		...
		const resetMainForm = () => {
			...
			$mainForm[0].reset();
			$mainForm.find('input[name="stn_action"]').val('add_note');
		...
		$('.stn-open-modal').on('click', (e) => {
			e.preventDefault();
			openModal();
		});
		$('.stn-modal__close, .stn-modal__backdrop').on('click', (e) => {
			e.preventDefault();
			closeModal();
		});

		// Toggle overlay handler
		$('.stn-toggle-overlay').on('click', (e) => {
			e.preventDefault();
			ajax({ action: 'stn_toggle_overlay' }, (resp) => {
				if (resp && resp.overlay_enabled !== undefined) {
					const $canvas = getCanvas();
					if (resp.overlay_enabled) {
